 ...
 */
export function replaceHexCharacters(text: string): string {
	...
	return sentences.reduce(function (accumulator, sentence) {
		if (sentence.length > 1) {
			...
			if (arrayOfNumbers.includes(characters[0]) && arrayOfNumbers.includes(characters[1])) {
				...
				return accumulator + String.fromCharCode(finalNumber) + sentence.substring(numberOfDigits);
			}
		}
		return accumulator + separator + sentence;
	});
}
